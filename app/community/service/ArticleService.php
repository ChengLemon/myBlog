...
class ArticleService
{
    ...
    public function adminPostList($filter, $isPage = false)
    {
        ...
        if (!empty($category)) {
            ...
            ]); // cmf_community_category_article
        ...
        $articles        = $communityArticleModel->alias('a')->field($field)
            ...
            ->paginate(10);
        // return $communityArticleModel->getLastSql();
    ...
    public function publishedArticle($postId, $categoryId = 0)
    {
        ...
        if (empty($categoryId)) {
            ...
            $article = $communityArticleModel->alias('post')->field('post.*')
                ->where($where)
                ->find();
        } else {
            $where = [
            ...
            $join    = [
                ['__COMMUNITY_CATEGORY_ARTICLE__ relation', 'post.id = relation.post_id']
            ];
            $article = $communityArticleModel->alias('post')->field('post.*')
                ->join($join)
                ->where($where)
