...
class CommunityCategoryModel extends Model {
    ...
    protected $pk = 'id';
    protected $name = 'community_category';
    protected $type = [
        'more' => 'array',
    ]; 
    ...
    /**
     * 获取全部分类或某一分类
     * @param type $categoryId  具体分类的id
     * @return type
     */
    public function getCate($categoryId = null) {
        if (!empty($categoryId)) {
...
//        }
    ...
    public function addCategory($data) {
        ...
        try {
            ...
            $this->where(['id' => ['neq', 0]])->update([ 'risshow' => 0 ]); // 修改已右侧边栏显示的分类
        ...
        if ($result != false) {
            ...
            if (!empty($data['alias']) && !empty($id)) {
                $routeModel->setRoute($data['alias'], 'community/List/index', ['id' => $id], 2, 5000);
                $routeModel->setRoute($data['alias'] . '/:id', 'community/Article/index', ['cid' => $id], 2, 4999);
            }
    ...
    public function editCategory($data) {
        ...
        if (empty($oldCategory) || empty($newPath)) {
            ...
            $routeModel = new RouteModel();
            if (!empty($data['alias'])) {
                $routeModel->setRoute($data['alias'], 'community/List/index', ['id' => $data['id']], 2, 5000);
                $routeModel->setRoute($data['alias'] . '/:id', 'community/Article/index', ['cid' => $data['id']], 2, 4999);
            } else {
                $routeModel->deleteRoute('community/List/index', ['id' => $data['id']]);
                $routeModel->deleteRoute('community/Article/index', ['cid' => $data['id']]);
            }
