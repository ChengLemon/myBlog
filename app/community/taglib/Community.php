...
namespace app\community\taglib;

use think\template\TagLib;

class Community extends TagLib {
    ...
    protected $tags = [
        ...
        'articles' => ['attr' => 'field,where,limit,order,page,relation,returnVarName,pageVarName,categoryIds', 'close' => 1],
        'breadcrumb'       => ['attr' => 'cid', 'close' => 1],//非必须属性self 
        'categories' => ['attr' => 'order,where', 'close' => 1], // 非必须属性item
        'subcategories' => ['attr' => 'categoryId', 'close' => 1], // 非必须属性item 
        'allsubcategories' => ['attr' => 'categoryId', 'close' => 1],//非必须属性item
        'category' => ['attr' => 'artid', 'close' => 1], //非必须属性item
    ];
    ...
    public function tagArticles($tag, $content) {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\community\service\ApiService::articles([
    'field'   => '{$field}',
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => '{$order}',
...
parse;
    ...
    public function tagCategory($tag, $content) {
        $item = empty($tag['item']) ? 'vv' : $tag['item']; //循环变量名
        $returnVarName = 'community_category_data';
  
        $artId = "0";
        if (!empty($tag['artid'])) {
            if (strpos($tag['artid'], '$') === 0) {
                $artId = $tag['artid'];
                $this->autoBuildVar($artId);
            } else {
                $artId = intval($tag['artid']);
                $artId = "{$artId}";
            }
        }
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\community\service\ApiService::artCategory({$artId});
 ?> 
...
parse;
    ...
    public function tagCategories($tag, $content) {
        ...
        $returnVarName = 'community_categories_data';
        $where         = '""';
        if (!empty($tag['where']) && strpos($tag['where'], '$') === 0) {
            $where = $tag['where'];
        }
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\community\service\ApiService::categories([
    'where'   => {$where},
    'order'   => '{$order}'
]);
...
parse;
    ...
    public function tagSubCategories($tag, $content) {
        $item = empty($tag['item']) ? 'vo' : $tag['item']; //循环变量名
        $returnVarName = 'community_sub_categories_data';
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\community\service\ApiService::subCategories({$categoryId});
